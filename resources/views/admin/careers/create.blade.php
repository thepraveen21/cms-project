...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-900">Create New Career</h2>
        ...
        <form action="{{ route('admin.careers.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="qualification" id="qualification" rows="4" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('qualification') }}</textarea>
                @error('qualification')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
